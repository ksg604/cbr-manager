...
##### Intel graphic card
1. Open Android studio and go to Configure>SDK manager>![SDK Update sites](/readme_images/build-setup03.PNG)
2. Select and install Intel HAXM
3. Follow the instruction, after installation you can check if it is installed correctly by typing `sc query intelhaxm` on the Window ![command prompt](/readme_images/build-setup04.PNG), if you can see `STATE 4 RUNNING` it mean the installation run correctly.
